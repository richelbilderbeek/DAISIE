DAISIE_sim_core = function(time,mainland_n,pars)
{
  ...
  while(timeval < time)
  {
    ...
    testit::assert(all.equal(ext_rate, DAISIE_calc_clade_ext_rate(ps_ext_rate = mu, n_species = length(island_spec[,1]))))
    testit::assert(all.equal(ana_rate, DAISIE_calc_clade_ana_rate(ps_ana_rate = laa, n_immigrants = length(which(island_spec[,4] == "I")))))
    testit::assert(all.equal(clado_rate, DAISIE_calc_clade_clado_rate(ps_clado_rate = lac, n_species = length(island_spec[,1]), carr_cap = K)))
    testit::assert(all.equal(immig_rate, DAISIE_calc_clade_imm_rate(ps_imm_rate = gam, n_island_species = length(island_spec[,1]), n_mainland_species = mainland_n, carr_cap = K)))
